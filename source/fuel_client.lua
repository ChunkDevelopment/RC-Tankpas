...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		local extraCost = fuelToAdd / 0.7 * Config.CostMultiplier
...
local tankpas = {
	{label = 'Ja', value = 'tankpas_gebruiken'},
	{label = 'Nee', value = 'tankpas_niet_gebruiken'}
}

function Tankpas()
	isMenuOpen = true

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tankpas', {
		...
		menu.close()
		isMenuOpen = false
		if data.current.value == 'tankpas_gebruiken' then
			currentCost = 0.0
			TriggerServerEvent('fuel:pay', currentCost) 
		end
		if data.current.value == 'tankpas_niet_gebruiken' then
			TriggerServerEvent('fuel:pay', currentCost)
			currentCost = 0.0
		end
		Citizen.Wait(10)
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			if Config.UseESX then
				extraString = "\n" .. Config.Strings.TotalCost .. ": ~r~€" .. Round(currentCost, 1)
			end
